...
#a <- cases %>% select(-death_per_case, -cases_per_10000, -deaths_per_10000, - county_name ,- state)
#corrplot(cor(a))
...
library(seriation)
#cm <- cor(cases_sel %>% na.omit)
#hmap(cm, margins = c(253,253))

cases %>% group_by(state) %>% 
...
dim(cases_train)
cases_train %>%  chi.squared(bad ~ .-county_name -state, data = .) %>% 
  arrange(desc(attr_importance)) %>% head(n = 22)

library(caret)
fit <- cases_train %>% train(bad ~ . - county_name - state,
                             method = "nb",
                             data = .,
                             tuneLength = 5,
...
# fit <- cases_train %>%
#   naive_bayes(bad ~ . - county_name - state,
#         data = . ,usekernel = T) 
fit
varImp(fit)
...
cases_train %>%  chi.squared(bad ~ ., data = .) %>% 
  arrange(desc(attr_importance)) %>% head(n = 20)
...
confusionMatrix(data = cases_test$bad_predicted, ref = cases_test$bad)
library(pROC)
roc_score = roc(cases_test$bad,cases_test$bad_predicted)  # AUC score
