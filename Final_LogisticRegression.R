...
cases_train <- cases_train %>% select(-death_per_case, -cases_per_10000, -deaths_per_10000,- county_name ,- state)
cases_test <- cases_test %>% select(-death_per_case, -cases_per_10000, -deaths_per_10000,- county_name, - state)
dim(cases_train)
cases_train %>%  chi.squared(bad ~ ., data = .) %>% 
  arrange(desc(attr_importance)) %>% head(n = 10)
...
varImp(fit)
cases_test <- cases_test %>% na.omit
cases_test$bad_predicted <- predict(fit, cases_test, type="response")
counties_test <- counties %>% left_join(cases_test %>% 
                                          mutate(county = county_name %>% str_to_lower() %>% 
                                                   str_replace('\\s+county\\s*$', '')))
ggplot(counties_test, aes(long, lat)) + 
